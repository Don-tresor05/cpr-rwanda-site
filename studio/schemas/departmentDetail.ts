...
export const departmentDetail = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      description:
        'A single short paragraph under "About <Department>". Only used when "Full Article" below is empty — fill in the Full Article tab instead for a longer, multi-paragraph write-up with headings and lists.',
      type: "localizedText",
      group: "main",
    }),
    defineField({
    ...
    ...localizedArticleBodyFields(
      "body",
      "Full Article",
      "The complete department write-up — as many paragraphs, headings and bullet lists as you need. When the English one has content it replaces the short Overview paragraph and Key Activities checklist above. Leave empty to keep using those simpler fields instead.",
    ),
    defineField({
      name: "headName",
      title: "Full name",
