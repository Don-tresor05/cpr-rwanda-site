...
function StatCard({ stat, active }: { stat: Stat; active: boolean }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-5 sm:p-8 bg-white/5 backdrop-blur-md border border-white/10 rounded-lg shadow-lg hover:bg-white/10 hover:border-white/20 transition-all duration-300 hover:scale-105 group h-full justify-center">
      ...
      <div className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white font-['Outfit'] tracking-tight mt-2">
        {count}{stat.suffix}
      </div>
      <div className="text-[10px] sm:text-xs text-white/70 uppercase tracking-widest font-bold text-center mt-1">{stat.label}</div>
    </div>
  );
}
...
export function StatsSection() {
  ...
  return (
    <section ref={ref} className="bg-[#4E6132] py-14 sm:py-20">
