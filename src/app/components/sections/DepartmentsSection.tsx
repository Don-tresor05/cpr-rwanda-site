...
import { ArrowRight, Loader2, Briefcase, Crown, GraduationCap, Handshake, Coins, Users, Scale, Radio } from "lucide-react";
import { getDepartments } from "../../data/departments";
import { useDepartments } from "../../hooks/useWordPress";
...
export function DepartmentsSection() {
  ...
  const { data: wpDepts, loading } = useDepartments();
  const fallbackDepts = useMemo(() => getDepartments(t), [t]);
  ...
  }, [wpDepts, fallbackDepts]);

  return (
    <section id="departments" ref={ref} className="py-24 bg-[#F8F9FA] relative">
            ...
            {departments.map((dept, i) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="font-['Outfit'] font-bold text-[#4E6132] text-lg mb-2 leading-tight" dangerouslySetInnerHTML={{ __html: dept.title }} />
                  <p className="text-[#4A4A4A] text-sm leading-relaxed mb-5 line-clamp-3" dangerouslySetInnerHTML={{ __html: dept.desc }} />
                  <Link
                    to={dept.link}
